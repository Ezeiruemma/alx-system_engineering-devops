...
- [x] The requirements below must be applied to `web-01` (feel free to do it on `lb-01` and `web-02`, but it won’t be checked)
- [x] Configure `ufw` so that it blocks all incoming traffic, except the following TCP ports:
  - [x] `22` (SSH)
  - [x] `443` (HTTPS SSL)
  - [x]x`80` (HTTP)
- [x] Share the `ufw` commands that you used in your answer file
...
- [x] Configure `web-01` so that its firewall redirects port `8080/TCP` to port `80/TCP`.
...
- [x] My web server `nginx` is only listening on port `80`
- [x] `netstat` shows that nothing is listening on `8080`
Terminal in `web-02`:
